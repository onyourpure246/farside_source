...
router.post('/verify', async (c) => {
    try {
        ...
        // Check if we need to sync with CAD API (Cache expiration: 24 hours)
        const CACHE_MS = 24 * 60 * 60 * 1000;
        const needsSync = !user || (Date.now() - new Date(user.updated_at).getTime() > CACHE_MS);
        ...
        if (needsSync) {
            ...
            if (!user) {
                ...
                // CASE B: User Exists -> Sync latest info from HR
