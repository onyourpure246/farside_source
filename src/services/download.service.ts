...
export class DownloadService {
	...
	async createFile(data: CreateFileRequest): Promise<DLFile> {
		...
		const result = await execute(
			`INSERT INTO dl_files (parent, category_id, name, description, filename, sysname, mui_icon, mui_colour, isactive, downloads, version, created_by, updated_by, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, NOW(), NOW())`,
			[data.parent || null, data.category_id || null, data.name, data.description || null, data.filename, data.sysname, muiIcon, muiColour, isactive, data.version || null, createdBy, createdBy]
		);
